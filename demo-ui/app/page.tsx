...
const BaseStatsSummary = ({ pokemon }: { pokemon: Pokemon }) => (
  <div className={styles.baseStatsSummary}>
    <div className={styles.textCenter}>HP: {pokemon.hp}</div>
    <div className={styles.textCenter}>こうげき: {pokemon.attack}</div>
    <div className={styles.textCenter}>ぼうぎょ: {pokemon.defense}</div>
    <div className={styles.textCenter}>とくこう: {pokemon.spAttack}</div>
    <div className={styles.textCenter}>とくぼう: {pokemon.spDefense}</div>
    <div className={styles.textCenter}>すばやさ: {pokemon.speed}</div>
  </div>
);
...
const WheelNumberInput = ({
  ...
  return (
    <input
      ...
      className={styles.wheelNumberInput}
    />
  );
};
...
const DamageBar = ({
  ...
  return (
    <div className={styles.damageBarContainer}>
      ...
      {/* バー本体 */}
      <div className={styles.damageBarTrack}>
        {/* 残りHP（緑） */}
        <div
          className={styles.damageBarFill}
          style={{
        ...
        <div
          className={styles.damageBarRange}
          style={{
...
const simulateSurvival = (maxHp: number, damage: number, item: Item) => {
  ...
  const calculateResult = () => {
    ...
    if (selectedMove.category === "変化") {
      return { min: 0, max: 0, hp: 100, effectiveness: 0, ohkoProb: 0, isStatus: true, survival: { minHits: 0, maxHits: 0 }, damageList: [], critDamageList: [], isDeadByEntry: false };
    }
    ...
    // 💀 0. 設置技（ステロ・まきびし）の事前計算（確定一発の判定基準にするため）
    let entryDamage = 0;
    const isGrounded = defender.type1 !== "ひこう" && defender.type2 !== "ひこう" && defAbility !== "ふゆう";
    if (defAbility !== "マジックガード") {
      if (isStealthRock) {
        const r1 = TYPE_CHART["いわ"]?.[defender.type1] ?? 1.0;
        const r2 = defender.type2 ? (TYPE_CHART["いわ"]?.[defender.type2] ?? 1.0) : 1.0;
        entryDamage += Math.floor(finalHp * (1 / 8) * r1 * r2);
      }
      if (spikes > 0 && isGrounded) {
        entryDamage += Math.floor(finalHp * (spikes === 1 ? 1/8 : spikes === 2 ? 1/6 : 1/4));
      }
    }
    const startHp = finalHp - entryDamage;
    const isDeadByEntry = startHp <= 0;
    ...
    // 🎲 ダメージ計算の内部関数（通常時と急所時で2回呼び出すため共通化）
    const calcDamageArray = (isCrit: boolean) => {
      // 💡 急所時は、自分のA下降ランクと相手のB上昇ランクを無視する
      const effAtkRank = (isCrit && atkRank < 0) ? 0 : atkRank;
      const effDefRank = (isCrit && defRank > 0) ? 0 : defRank;

      // 1. 攻撃力 (A) の計算
      let a = calcStat(isPhysical ? attacker.attack : attacker.spAttack, atkEv, atkNature);
      a = Math.floor(a * RANK_MODIFIERS[effAtkRank]);
      ...
      d = Math.floor(d * RANK_MODIFIERS[effDefRank]);
      ...
      // ミストフィールドのドラゴン技半減効果は、地面にいるポケモンにのみ適用される
      ...
      for (let i = 85; i <= 100; i++) {
        ...
        // 💡 おやこあい、テクニシャンの処理をループ内に安全に配置
        if (atkAbility === "おやこあい") dmg = Math.floor(dmg * 1.25); // 2回攻撃の合計概算
        if (atkAbility === "テクニシャン" && selectedMove.power <= 60) dmg = Math.floor(dmg * 1.5);

        // 特性補正：マルチスケイルはHP満タンのときのみダメージ半減
        if (defAbility === "マルチスケイル" && isFullHp && entryDamage === 0) dmg = Math.floor(dmg * defAbil.multiplier);
      ...
      return { list: dList, effectiveness };
    };

    // 💡 通常ダメージと急所ダメージを両方計算する
    const normalData = calcDamageArray(false);
    const critData = calcDamageArray(true);

    // 💡 一撃確率は「ステロダメージを引いた後のHP（startHp）」で判定する
    const ohkoCount = normalData.list.filter((d) => d >= startHp).length;
    const ohkoProb = isDeadByEntry ? 100 : (ohkoCount / 16) * 100;

    const minHits = simulateSurvival(finalHp, normalData.list[15], defItem); 
    const maxHits = simulateSurvival(finalHp, normalData.list[0], defItem);

    return {
      min: normalData.list[0], max: normalData.list[15], hp: finalHp,
      effectiveness: normalData.effectiveness, ohkoProb, isStatus: false,
      damageList: normalData.list, // 通常ダメージリスト（getSurvivalText用）
      critDamageList: critData.list, // 急所ダメージリスト（UI表示用）
      survival: { minHits, maxHits },
      isDeadByEntry // ステロ等で死ぬかのフラグ（バッジ表示用）
    };
  };
  ...
  return (
    <main className={styles.mainContainer}>
      ...
      <div className={styles.panelsWrapper}>
        {/* 攻撃側設定 */}
        <section className={styles.pokemonPanel}>
          <h2 className={styles.panelTitleAtk}>
            攻撃側
          </h2>
          <PokemonSearch
            label="攻撃側"
            ...
            <div className={styles.panelContent}>
              ...
              <div className={styles.inputGroup}>
                ...
                <div className={styles.sliderHeader}>
                  <label className={styles.sliderLabelText}>努力値 (0-32)</label>
                  <span className={styles.sliderValueTextAtk}>
                    {atkEv}pt
                  </span>
                </div>
              ...
              {(() => {
                ...
                return (
                  <div className={styles.actualStatBox}>
                    <p className={styles.actualStatBoxTitle}>
                      使用ステータス: {label}
                    </p>
                    <p className={styles.actualStatBoxText}>
                      種族値: {baseStat} →{" "}
                      <span className={styles.actualStatHighlight}>
                        実数値: {actualStat}
                      </span>
                    </p>
                  </div>
              ...
              <div className={styles.natureWrapper}>
                <label className={styles.natureLabel}>
                  性格補正
                </label>
                <div className={styles.natureRadioGroup}>
                  {[
                    ...
                    <label
                      key={opt.value}
                      className={styles.natureRadioLabel}
                    >
                      <input
                        type="radio"
              ...
              <div className={styles.rankWrapper}>
                <label className={styles.rankLabel}>
                  ランク
                </label>
                ...
                {/* よく使うクイックボタン */}
                <div className={styles.quickRankGroup}>
                  <button
                ...
                <span className={styles.rankValueAtk}>
              ...
              <div className={styles.statusWrapper}>
                ...
                {isAtkBurned && atkAbility === "こんじょう" && (
                  <span className={styles.gutsBadge}>
                    こんじょう発動(1.5x)
                  </span>
                )}
              </div>
                ...
                {selectedMove && (
                  ...
                  >
                    <div className={styles.moveHeader}>
                      ...
                      {/* 👇 威力と命中を並べて表示 */}
                      <div className={styles.moveStatsWrapper}>
                        <span className={styles.movePowerText}>
                          威力: {selectedMove.power && selectedMove.power > 0 ? selectedMove.power : "—"}
                        </span>
                        <span className={styles.moveAccText}>
                          命中: {selectedMove.accuracy ? selectedMove.accuracy : "—"}
                        </span>
                      </div>
                    </div>

                    <div className={styles.moveMetaInfo}>
...
{/* === 中央エリア（入れ替えボタン ＆ 天候・フィールド） === */}
        <div className={styles.centerArea}>
          ...
          <div className={styles.envPanel}>
            {/* 天候 */}
            <div>
              <span className={styles.envSectionTitle}>☀️ 天候</span>
              <div className={styles.envOptionGroup}>
                {[
                  { label: "なし", value: "none" },
                  { label: "晴れ", value: "sun" },
                  { label: "雨", value: "rain" },
                  ...
                  <label key={w.value} className={styles.envRadioLabel}>
                    <input
                      type="radio"
                      name="weather"
                      value={w.value}
            ...
            {/* フィールド */}
            <div>
              <span className={styles.envSectionTitle}>🌱 フィールド</span>
              <div className={styles.envOptionGroup}>
                {[
                  { label: "なし", value: "none" },
                  ...
                  <label key={t.value} className={styles.envRadioLabel}>
                    <input
                      type="radio"
                      name="terrain"
                      value={t.value}
...
<div className={styles.envDivider}>
              <span className={styles.envSectionTitleLarge}>🪨 設置技・状態異常</span>
              
              <div className={styles.hazardsGroup}>
                
                {/* ステルスロック */}
                <label className={styles.hazardCheckboxLabel}>
                  <input
                    type="checkbox"
                ...
                {/* まきびし */}
                <div className={styles.hazardSubGroup}>
                  <span className={styles.hazardSubTitle}>まきびし:</span>
                  {[0, 1, 2, 3].map((num) => (
                    <label key={`spikes-${num}`} className={styles.envRadioLabel}>
                      <input
                        type="radio"
                        name="spikes"
                        value={num}
                ...
                {/* 毒状態（排他選択） */}
                <div className={styles.hazardSubGroup}>
                  <span className={styles.hazardSubTitle}>毒状態:</span>
                  <label className={styles.envRadioLabel}>
                    <input 
                      type="radio" 
                      name="poison" 
        ...
        {/* 防御側設定 */}
        <section className={styles.pokemonPanel}>
          <h2 className={styles.panelTitleDef}>
            防御側
          </h2>
          <PokemonSearch
            label="防御側"
          ...
          {defender && (
            <div className={styles.panelContent}>
              ...
              <label className={styles.sectionLabelDef}>
                HP 努力値
              </label>
              <div className={styles.inputGroup}>
                {/* ラベルと数値の表示 */}
                <div className={styles.sliderHeader}>
                  <label className={styles.sliderLabelTextDef}>努力値 (0-32)</label>
                  <span className={styles.sliderValueTextDef}>
                    {defHpEv}pt
                  </span>
                </div>
              ...
              {/* HPの実数値表示 */}
              <div className={styles.actualStatBoxDefHp}>
                <p className={styles.actualStatBoxDefTitle}>HP</p>
                <p className={styles.actualStatBoxDefText}>
                  種族値: {defender.hp} →{" "}
              ...
              <label className={styles.sectionLabelDefTop}>
                防御側の耐久努力値
              </label>
              <div className={styles.inputGroup}>
                {/* ラベルと数値の表示 */}
              ...
              {(() => {
                ...
                return (
                  <div className={styles.actualStatBoxDefAlt}>
                    <p className={styles.actualStatBoxDefTitle}>{label}</p>
                    <p className={styles.actualStatBoxDefText}>
                      種族値: {baseStat} →{" "}
                      <span style={{ fontWeight: "bold" }}>
                        実数値: {actualStat}
              ...
              <div className={styles.natureWrapper}>
                ...
                <div className={styles.natureRadioGroup}>
                  {[
                    ...
                    <label key={opt.value} className={styles.natureRadioLabel}>
                      <input
                        type="radio"
              ...
              <div className={styles.rankWrapper}>
                ...
                <span className={styles.rankValueDef}>
